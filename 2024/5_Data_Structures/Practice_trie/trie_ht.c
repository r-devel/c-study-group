...
// Root of trie
node *root; // declare root once and don't touch it again
...
bool check(char *word)
{
    ...
    node *cursor = root; // declared globally
    ...
    for (int i = 0, n = strlen(word); i < n; i++)
    {
        ...
        // check if children[index] is a NULL pointer
        if (cursor->children[index] == NULL)
        {
        ...
        {
            // reset cursor to this node
            cursor = cursor->children[index];
            // this won't compile - doesn't work for zero-length word?
            // if (i == (n - 1)) return cursor->is_word;
        }
    }
    // got to end of word - is is_word true?
